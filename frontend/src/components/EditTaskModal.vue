...
watch(
  () => tasksStore.toEditTask,
  (val) => {
    if (val) {
      name.value = val.name ?? ''
      reloadType.value = val.reload_type ?? 'daily'
      resetTime.value = val.reset_time ?? '03:00'
      resetWeekday.value = val.reset_weekday ?? 0
      reloadEvery.value = val.reload_every ?? 1
    }
  },
  { immediate: true },
)
  ...
  <transition name="fade-scale">
    <div v-if="show" class="fixed inset-0 z-20 flex items-center justify-center">
      ...
      <form
        ...
        class="bg-neutral-800 rounded-lg border border-neutral-700 p-6 w-105 transition-transform z-30"
      >
        <h1 class="text-2xl font-semibold mb-2 text-white font-unbounded">Редактировать задачу</h1>
        ...
        <div class="mt-6 flex justify-end gap-2">
          <button
            type="button"
            @click="tasksStore.toggleIsEditModalOpen()"
